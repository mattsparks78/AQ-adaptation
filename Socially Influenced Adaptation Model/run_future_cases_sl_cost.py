...
# Create dict to hold net_ben values for each case
net_ben_dict = {}
# Set year, either 2050 or 2100
for k_factor in np.linspace(0.5, 1.5, 11):
    net_ben_dict[k_factor] = {}
    for cost_factor in np.linspace(0.3, 1, 8):
        net_ben_dict[k_factor][cost_factor] = {}
        for risk_factor in np.linspace(0.5, 1.5, 11):    
            ...
            for year in [2050, 2100]:
                ...
                for policy in ['REF', 'P45', 'P37']:
                    ...
                    for IC in ['IC' + str(i) for i in range(1,6)]:
                        ...
                        indir = './input'
                        indir1 = r"C:\Users\User\OneDrive - University of Waterloo\Sparks\PNAS\WorkingFolder\Data\Social Learning Model\\"
                        outdir = r"C:\Users\User\OneDrive - University of Waterloo\Sparks\PNAS\WorkingFolder\Data\Social Learning Model\Figure 4\\"
                        ...
                        if year == 2050:
                            ...
                            x_df = pd.read_csv(indir1+'/x_'+in_file+'.csv', index_col=0, parse_dates=True)
                            x0 = x_df[-1:].to_dict(orient='list')
                        ...
                        mean_wage = sum([wages[k]*(pop_dict[k]/sum(pop_dict.values())) for k in pop_dict.keys()]) * cost_factor
...
with open(outdir + 'net_ben_dict_sl_cost_risk', 'wb') as f:
