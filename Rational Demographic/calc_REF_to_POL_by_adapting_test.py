        ...
        for Yr in years:
            ...
            for IC in IC_list:
                ...
                for r, c in rc:
                    ...
                    if pol == 'P37':
                        pm_target = np.mean(pm_out_P37[str(Yr)][IC][(r, c)])*corr
                    elif pol == 'P45':
                        pm_target = np.mean(pm_out_P45[str(Yr)][IC][(r, c)])*corr
                    
                    # Calculate baseline mean outdoor PM concentration
                    pm_o = np.mean(pm_out_REF[str(Yr)][IC][(r, c)])*corr
                    ...
                    for race in ['White', 'Black', 'Asian', 'Native American', 'Pacific', 'Other', 'Two Plus']:
                        ...
                        pm_sort = np.sort(pm_out_REF[str(Yr)][IC][(r, c)]*corr)
...
# with open(outdir + 'adapt_ben_all_REF_to_POL_test_' + str(inf_rate), 'wb') as f:
#     cp.dump(adapt_ben, f)
# f.close()

# with open(outdir + 'adapt_ben_pc_all_REF_to_POL_test_' + str(inf_rate), 'wb') as f:
#     cp.dump(adapt_ben_pc, f)
# f.close()

# with open(outdir + 'adapt_cost_all_REF_to_POL_test_' + str(inf_rate), 'wb') as f:
#     cp.dump(adapt_cost, f)
# f.close()

# with open(outdir + 'adapt_cost_pc_all_REF_to_POL_test_' + str(inf_rate), 'wb') as f:
#     cp.dump(adapt_cost_pc, f)
# f.close()

# with open(outdir + 'adapt_count_all_REF_to_POL_test_' + str(inf_rate), 'wb') as f:
#     cp.dump(adapt_count, f)
# f.close()

# with open(outdir + 'adapt_util_all_REF_to_POL_test_' + str(inf_rate), 'wb') as f:
