...
#%%
# Create set of F_INF values to try
finf_list = [0, 0.05, 0.1, 0.2, 0.3, 0.4, 0.51, 0.75]
for finf in finf_list:
    # Set infiltration rate based on empirical data/for sensitvity analysis
    inf_rate = finf

    # Calculate ratio of non-adaptation exposure in current finf to non-adaptation exposure in base finf
    exp_ratio = (prop_out + prop_in * inf_rate) / (prop_out + prop_in * finf_base) 
    
    
    # Calculate ratio of adaptation exposure to non-adaptation exposure
    ...
    for years in year_set:
        ...
        for Yr in years:
            ...
            for IC in IC_list:
                ...
                for r, c in rc:
                    pm_out_cell_finf = pm_out[str(Yr)][IC][(r, c)] * exp_ratio
                    # Calculate baseline mean outdoor PM concentration
                    pm_o = np.mean(pm_out_cell_finf)
                    ...
                    pm_sort = np.sort(pm_out_cell_finf)
                    ...
                    adapt_ben[Yr][IC][(r, c)] = base_mort * AF * \
                    ...
                    adapt_util[Yr][IC][(r, c)] = adapt_ben[Yr][IC][(
                        r, c)] - adapt_cost[Yr][IC][(r, c)]
                    adapt_util_pc[Yr][IC][(r, c)] = adapt_ben_pc[Yr][IC][(
                        r, c)] - adapt_cost_pc[Yr][IC][(r, c)]
    
    
    # # Output pickles
    # with open(outdir + 'adapt_ben_all_P45_' + str(inf_rate), 'wb') as f:
    #     cp.dump(adapt_ben, f)
    # f.close()
    
    # with open(outdir + 'adapt_ben_pc_all_P45_' + str(inf_rate), 'wb') as f:
    #     cp.dump(adapt_ben_pc, f)
    # f.close()
    
    # with open(outdir + 'adapt_cost_all_P45_' + str(inf_rate), 'wb') as f:
    #     cp.dump(adapt_cost, f)
    # f.close()
    
    # with open(outdir + 'adapt_cost_pc_all_P45_' + str(inf_rate), 'wb') as f:
    #     cp.dump(adapt_cost_pc, f)
    # f.close()
    
    # with open(outdir + 'adapt_count_all_P45_' + str(inf_rate), 'wb') as f:
    #     cp.dump(adapt_count, f)
    # f.close()
    
    # with open(outdir + 'adapt_util_all_P45_' + str(inf_rate), 'wb') as f:
    #     cp.dump(adapt_util, f)
    # f.close()
    
    # with open(outdir + 'adapt_util_pc_all_P45_' + str(inf_rate), 'wb') as f:
    #     cp.dump(adapt_util_pc, f)
    # f.close()
    
    # with open(outdir + 'PM_out_base_P45_' + str(inf_rate), 'wb') as f:
    #     cp.dump(pm_out_base, f)
    # f.close()
    
    # with open(outdir + 'PM_out_adapt_P45_' + str(inf_rate), 'wb') as f:
