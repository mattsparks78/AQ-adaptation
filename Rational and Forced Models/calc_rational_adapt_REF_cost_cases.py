    ...
    for Yr in years:
        ...
        for IC in IC_list:
            ...
            for r, c in rc:
                ...
                for case in ['wage', 'low wage', 'high wage', 'low rec', 'mean rec', 'high rec']:
                    if case == 'wage':
                        # Find and store mean hourly wage for the current grid cell
                        w_i = wage.loc[(wage['ROW'] == r) & (
                            wage['COL'] == c), '2000'].item() * EPF * w_conv
                    if case == 'low wage':
                        w_i = min(wage['2000']) * EPF * w_conv
                    if case == 'high wage':
                        w_i = max(wage['2000']) * EPF * w_conv
                    if case == 'low rec':
                        w_i = (13.54 / AT) * EPF * w_conv
                    if case == 'mean rec':
                        w_i = (49.05 / AT) * EPF * w_conv
                    if case == 'high rec':
                        w_i = (100.91 / AT) * EPF * w_conv
                    ...
                    adapt_count[Yr][IC][(r, c)][case] = count
    
                    # Output the PM_out after adaptation
                    pm_out_adapt[Yr][IC][(r, c)][case] = np.mean(pm_descend)
    
                    # Output adaptation benefits
                    popu = pop.loc[(pop['ROW'] == r) & (
                        pop['COL'] == c), '2000'].item() * PPF
                    
                    adapt_ben[Yr][IC][(r, c)][case] = base_mort * AF * \
                        dPM * 0.1 * popu * ppa * VSL
                        
                    adapt_ben_pc[Yr][IC][(r, c)][case] = base_mort * AF * dPM * 0.1 * VSL
    
                    # Output adaptation costs
                    adapt_hours = count * AT
                    adapt_cost[Yr][IC][(r, c)][case] = adapt_hours * w_i * popu * ppa
                    adapt_cost_pc[Yr][IC][(r, c)][case] = adapt_hours * w_i
    
                    # Output adaptation utility
                    adapt_util[Yr][IC][(r, c)][case] = adapt_ben[Yr][IC][(
                        r, c)][case] - adapt_cost[Yr][IC][(r, c)][case]
                    
                    adapt_util_pc[Yr][IC][(r, c)][case] = adapt_ben_pc[Yr][IC][(
                        r, c)][case] - adapt_cost_pc[Yr][IC][(r, c)][case]


# # Output pickles
# with open(outdir + 'adapt_ben_all_REF_cost_cases' + str(inf_rate), 'wb') as f:
#     cp.dump(adapt_ben, f)
# f.close()

# with open(outdir + 'adapt_ben_pc_all_REF_cost_cases' + str(inf_rate), 'wb') as f:
#     cp.dump(adapt_ben_pc, f)
# f.close()

# with open(outdir + 'adapt_cost_all_REF_cost_cases' + str(inf_rate), 'wb') as f:
#     cp.dump(adapt_cost, f)
# f.close()

# with open(outdir + 'adapt_cost_pc_all_REF_cost_cases' + str(inf_rate), 'wb') as f:
#     cp.dump(adapt_cost_pc, f)
# f.close()

# with open(outdir + 'adapt_count_all_REF_cost_cases' + str(inf_rate), 'wb') as f:
#     cp.dump(adapt_count, f)
# f.close()

# with open(outdir + 'adapt_util_all_REF_cost_cases' + str(inf_rate), 'wb') as f:
#     cp.dump(adapt_util, f)
# f.close()

# with open(outdir + 'adapt_util_pc_all_REF_cost_cases' + str(inf_rate), 'wb') as f:
#     cp.dump(adapt_util_pc, f)
# f.close()

# with open(outdir + 'PM_out_base_REF_cost_cases' + str(inf_rate), 'wb') as f:
#     cp.dump(pm_out_base, f)
# f.close()

# with open(outdir + 'PM_out_adapt_REF_cost_cases' + str(inf_rate), 'wb') as f:
