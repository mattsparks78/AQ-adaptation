...
outdir2 =  r"C:\Users\User\OneDrive - University of Waterloo\Sparks\PNAS\WorkingFolder\Figures\Rational\\"
...
# Output pickles 
with open(outdir + 'adapt_ben_all_P45', 'wb') as f:
      cp.dump(adapt_ben,f)
f.close()

with open(outdir + 'adapt_ben_pc_all_P45', 'wb') as f:
      cp.dump(adapt_ben_pc,f)
f.close()

with open(outdir + 'adapt_cost_all_P45', 'wb') as f:
      cp.dump(adapt_cost,f)
f.close()

with open(outdir + 'adapt_cost_pc_all_P45', 'wb') as f:
      cp.dump(adapt_cost_pc,f)
f.close()

with open(outdir + 'adapt_count_all_P45', 'wb') as f:
      cp.dump(adapt_count,f)
f.close()

with open(outdir + 'adapt_util_all_P45', 'wb') as f:
      cp.dump(adapt_util,f)
f.close()

with open(outdir + 'adapt_util_pc_all_P45', 'wb') as f:
      cp.dump(adapt_util_pc,f)
f.close()

with open(outdir + 'PM_out_base_P45', 'wb') as f:
      cp.dump(pm_out_base,f)
f.close()

with open(outdir + 'PM_out_adapt_P45', 'wb') as f:
...
for r, c in rc:
    ...
    for Yr in year_set[0]:
        for IC in IC_list:
            adapt_cell[(r, c)].append(adapt_count[Yr][IC][(r, c)])    
    


ac = adapt_cell[(8, 18)]
plt.hist(ac, bins = 20, density = True)
plt.hist(ac, bins = 20, density = True)
plt.xlabel('Days')
plt.ylabel('Probability')
plt.title('Annual Mean Adaptation Days for P45 2050 Cell (8, 18)')
plt.xlim(200, 300)
# plt.savefig(outdir2 + 'Adaptation Days Check.png', dpi = 400, bbox_inches = 'tight')
